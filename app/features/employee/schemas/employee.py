...
def _validate_dob_hire_relationship(
    date_of_birth: date | None, hire_date: date | None
) -> None:
...
class EmployeeCreateRequest(BaseModel):
    ...
    user_id: UUID = Field(
        description="The auth User account this employee profile belongs to."
    )
...
class EmployeeStatusUpdateRequest(BaseModel):
    ...
    employment_status: EmploymentStatus = Field(
        description="The new employment status."
    )
...
class EmployeeRestoreRequest(BaseModel):
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={
            "examples": [
                {"reason": "Reinstated after HR review of termination appeal."}
            ]
        },
    )
