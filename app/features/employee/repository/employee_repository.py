...
class EmployeeRepository:
    ...
    async def get_by_id_including_deleted(self, employee_id: uuid.UUID) -> Employee | None:
    ...
    async def count(
        ...
        result = await self.db.execute(select(func.count(Employee.id)).where(*conditions))
